...
if (in_array($file["type"],$type)) {
	...
	if ($size<=1024*5) {
		if (move_uploaded_file($file["tmp_name"],$filename)) {
			$sql=Controller::$conn->newsUpdate($filename,$basliq,$text,$status,$kate,$id,$basliqr,$textr,$basliqen,$texten);
		...
		$sql=Controller::$conn->newsUpdate($uImage,$basliq,$text,$status,$kate,$id,$basliqr,$textr,$basliqen,$texten);
	echo "<center><h1 class='text-primary'>Melumat deyisdi</h1></center>";
	echo "<script>
			setTimeout(function(){
				location.replace('".$site_url."news/'".$lng.");
